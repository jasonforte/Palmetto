...
class Method(list):
    ...
    def applyTo(self, input_image):
        ...
        for sequence in range(0, len(self)):
            ...
            # as the parameters
            op = self[sequence](result_image, options=self.options[sequence])
...
if __name__ == '__main__':
    ...
    import operations.CenterOfMass
    import operations.graynorm

    import time
    
    # select the sample image to transform
    sample_file = structure.Base.sample_dir + 'example.png'
    ...
    m1.append(operations.CenterOfMass.CenterOfMass, options={'window':(420, 380)})
    m1.append(operations.graynorm.GrayNorm, options={})
    m1.append(operations.blur.GaussBlur, options={})
    ...
    result1 = m1.applyTo(img)
    
    cv2.imshow('Original', img)
    cv2.imshow('Output of Process Tree', result1)
    cv2.waitKey(0)
